...
public class InitAcceptingReachableEncoding {
	...
	public InitAcceptingReachableEncoding(TransducerEncoding encoding) {
	...
	private void allocateBoolVars() {
		startIndexOfYVars = encoding.getNextSATVar();
		encoding.setNextSATVar(startIndexOfYVars + numStates * numStates);
					
		startIndexOfYPrimeVars = encoding.getNextSATVar();
		encoding.setNextSATVar(startIndexOfYPrimeVars + numStates * numStates);
		
		startIndexOfTempY = encoding.getNextSATVar();
		encoding.setNextSATVar(startIndexOfTempY + numStates * numStates * (numStates - 1));
		
		startIndexOfTempYPrime = encoding.getNextSATVar();
		encoding.setNextSATVar(startIndexOfTempYPrime + numStates * numStates * (numStates - 1));
	}
			...
			for(int qPrime = 1; qPrime <= numStates; qPrime++){
				for(int d = 1; d <= numStates - 1; d++){
					...
					for(int i = 0; i < transitionsFromQToQPrime.size(); i++){
						...
						innerClause[1] = -y;
						innerClause[2] = tempY;
						encoding.addClause(innerClause);
					}
				}
			}
		}
