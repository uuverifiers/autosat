 ...
 *
 */
public class InjectiveInput implements ConditionChecking{

	private Listener listener;
	public InjectiveInput(Listener listener) {
	...
	/**
	 * Return list of edges such that it contains an edge whose output1 is different from output2
	 * and it ends at an accepting state
	 * @param model
	 * @param numStates
	 * @param numLetters
	 ...
	 * @return
	 */
	private List<Integer>[] check(int[] model, int numStates, int numLetters, Set<Integer> acceptingStates){
		EdgeWeightedDigraph productGraph = productConstruction(TransducerGenerator.fromSATModel(model, numStates, numLetters, acceptingStates));
		List<DirectedEdge> edges = check(productGraph);
		
		listener.inform("Injective 1");
		listener.inform(String.valueOf(edges));
		return convert(edges);
	}
	
	public List<DirectedEdge> check(EdgeWeightedDigraph productGraph){
